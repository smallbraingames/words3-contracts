...
library GameConfig {
  ...
  function decodeStatic(
    bytes memory _blob
  ) internal pure returns (Status status, uint32 crossWordRewardFraction, uint16 bonusDistance, uint8 numDrawLetters) {
  ...
  function decode(
    ...
    (_table.status, _table.crossWordRewardFraction, _table.bonusDistance, _table.numDrawLetters) = decodeStatic(
      _staticData
    );
  }
