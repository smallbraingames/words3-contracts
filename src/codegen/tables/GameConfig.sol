...
ResourceId constant _tableId =
    ResourceId.wrap(bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("GameConfig"))));
ResourceId constant GameConfigTableId = _tableId;

FieldLayout constant _fieldLayout = FieldLayout.wrap(0x0025030001200400000000000000000000000000000000000000000000000000);
...
library GameConfig {
    ...
    function set(Status status, uint256 endTime, uint32 crossWordRewardFraction) internal {
        bytes memory _staticData = encodeStatic(status, endTime, crossWordRewardFraction);
     ...
     * @notice Set the full data using individual values.
     */
    function _set(Status status, uint256 endTime, uint32 crossWordRewardFraction) internal {
        bytes memory _staticData = encodeStatic(status, endTime, crossWordRewardFraction);
    ...
    function set(GameConfigData memory _table) internal {
        bytes memory _staticData = encodeStatic(_table.status, _table.endTime, _table.crossWordRewardFraction);
    ...
    function decodeStatic(bytes memory _blob)
        internal
        pure
        returns (Status status, uint256 endTime, uint32 crossWordRewardFraction)
    {
    ...
    {
        (_table.status, _table.endTime, _table.crossWordRewardFraction) = decodeStatic(_staticData);
    }
