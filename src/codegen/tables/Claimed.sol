...
bytes32 constant _tableId = bytes32(abi.encodePacked(bytes16(""), bytes16("Claimed")));
bytes32 constant ClaimedTableId = _tableId;
...
library Claimed {
  ...
  function get(IStore _store, address player) internal view returns (bool value) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(uint160(player)));
  ...
  function set(IStore _store, address player, bool value) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(uint160(player)));
