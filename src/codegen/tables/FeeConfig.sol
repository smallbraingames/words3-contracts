...
library FeeConfig {
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (FeeConfigData memory _table) {
   ...
   */
  function encode(address feeTaker, uint16 feeBps) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(feeTaker, feeBps);
