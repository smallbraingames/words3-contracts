...
contract LibBoardTest is MudV2Test {
    ...
    function testGetRewardPerEmptyTileFuzzEightLettersTwoEmpty(
        ...
        uint256 reward = LibBoard.getRewardPerEmptyTile(
            rawWordToWord(rawWord),
            rewardFraction,
            value
        );
        if (numEmpty == 0) {
    ...
    function testGetLetterCoordLeftFuzzOffset(
        int32 offset,
        int32 startX,
        int32 startY
    ) public {
    ...
    function testGetOutsideBoundCoords() public {
        (Coord memory start, Coord memory end) = LibBoard.getOutsideBoundCoords(
            Coord({x: 5, y: 8}),
            Direction.LEFT_TO_RIGHT,
            10,
            8
        ...
        (Coord memory startTop, Coord memory endTop) = LibBoard
    ...
    function testGetWordInBoundsCheckedFuzzNoWords(
        int32 x,
        int32 y,
        uint16 positive,
        uint16 negative
        ...
        wrapper.getWordInBoundsChecked(
            Coord({x: x, y: y}),
            Direction.TOP_TO_BOTTOM,
            positive,
            negative
...
contract WrapperContract {
    function getWordInBoundsChecked(
        ...
        LibBoard.getWordInBoundsChecked(
            letterCoord,
            direction,
            positive,
            negative
