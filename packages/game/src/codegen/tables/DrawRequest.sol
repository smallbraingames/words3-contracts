...
library DrawRequest {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "words3", name: "DrawRequest", typeId: RESOURCE_TABLE });`
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (DrawRequestData memory _table) {
   ...
   */
  function encode(address player, bool fulfilled) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(player, fulfilled);
